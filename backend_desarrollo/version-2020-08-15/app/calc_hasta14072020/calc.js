...
$scope.initData = function() {
  ...
  $scope.origen={observ:'',formapago:'BITAU', nombrebank:'Itaú', nrocta:'12345678901234567890', nombretitular:'Dina Osma', doctitular:'1234567',
  comprobantePago:''};
  ...
  $scope.setResumenOrigen = function() {
    ...
    //TODO. terminar. ahora
    $scope.datosFormaPago        = {};
    $scope.datosFormaPago        = $scope.getDatosBanco(formaPago);
    $scope.origen.nombrebank     = $scope.datosFormaPago.nombrebank;
    $scope.origen.nrocta         = $scope.datosFormaPago.nrocta;
    $scope.origen.nombretitular  = $scope.datosFormaPago.nombretitular;
    $scope.origen.doctitular     = $scope.datosFormaPago.doctitular;
    $scope.origen.observ         = $scope.datosFormaPago.observ;
    $scope.origen.descripcion    = $scope.datosFormaPago.descripcion;

    if (formaPago.charAt(0) == 'B'){ //banco
      console.log('Forma de pago es banco. Hay que mostrar resumen y todos los datos del banco');
      resumen    =  $scope.datosFormaPago.descripcion;
      ...
      */
      ...
      console.log('Forma de pago es Wester Union');
      resumen += 'Nombre Completo:    ' + $scope.origen.wu.nombretitular + '\n';
      resumen += 'Cédula/Pasaporte: ' + $scope.origen.wu.doctitular;
    }

    $scope.origenResumen = resumen;
    console.log('origenResumen: ' + $scope.origenResumen);
  };
...
/**/
  ...
  console.log('codigo de forma de pago: ' + codFormaPago);
  //solo venezuela
  ...
  .then(function (response) {
    ...
    $scope.DatosBanco = $scope.datos_banco;
...
/*
$scope.DatosPagoOrigen={observ:'', nombrebank:'Itau', nrocta:'12345678901234567890', nombretitular:'Dina Osma', doctitular:'1234567',
  comprobantePago:''};
  return $scope.DatosPagoOrigen;
*/
  
  return $scope.DatosBanco;
