...
$scope.save = function() {
  ...
  if ($scope.data.id_transaccion != null && $scope.data.id_transaccion != "" && $scope.data.id_transaccion != 0 ){
    metodo = "update transaccion";
    $scope.update(); //TODO. pendiente, actualizar transacción con datos de pasos 3,4,5... 
  }else{
      ...
      if (response.data){
        ...
        if(dataResp.resultado != null)
          if(dataResp.resultado == 'EXITO'){
            ...
            console.info('id_trans: ' + $scope.transaccion.id);
  ...
  $scope.update = function() {
    console.log('controlador-calc- funcion:update - inicio');
    $http.post('./calc/update_transaccion.php', JSON.stringify($scope.dataSave))
    .then(function (response) {
      //debugger;
      //+$scope.showErrorNotFound = false;
      if (response.data){
...
$scope.calcularMontoDestinoconComision = function(){
  ...
  //$scope.calcularMontoDestinoconComision();
    ...
    //valor por defecto: 1
    $scope.paso = 1; //1:calculo, 2:forma de pago origen, 3:enviar comprobante, 4: resumen, 5: datos de usuario
