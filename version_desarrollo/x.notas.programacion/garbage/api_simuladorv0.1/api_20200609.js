  ...
  $scope.getDataYadio = function() {
    ...
    .then(function(response) {
      ...
      if (response.data.records != null){
        ...
        $scope.showDataYadio();
        $scope.dataOk = true;
        $scope.calcularMontoUSD();
        $scope.calcularMontoBs();
      }
      else{
...
//calcular monto3 (monto a moneda destino) basado en la tasa en dolares
$scope.calcularMontoBs = function(){
  console.log('tasa USD/Bs: ' + $scope.USD_VES_rate);
  console.log('calc.monto2: ' + $scope.monto2);
  console.log('calc.USD_VES_rate: ' + $scope.USD_VES_rate);
...
$scope.calcularMonto1_A = function() {
  console.log('controlador:calc. monto3=' + $scope.data.monto3);
  var monto1 = $scope.monto3 * $scope.USD_PYG;
  $scope.monto1 = parseInt(monto1.toFixed(0));
